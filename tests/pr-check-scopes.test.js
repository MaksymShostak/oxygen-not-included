...
const SCOPES = ["sdlc", "pipeline", "mods"];

test("mod-only PRs do not unconditionally launch the SDLC control matrix", () => {
...
describe("native Git PR check selection", () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), "oni-pr-checks-"));
    environment = {
      ...Object.fromEntries(
  ...
  afterEach(() => {
    ...
    if (
      ...
      !basename(target).startsWith("oni-pr-checks-")
    ) {
  ...
  test.each([
    ...
    ["tools/oni-mod-pipeline/Program.cs", ["pipeline", "mods"]],
    ["mods/example/Mod.cs", ["mods"]],
    ["package-lock.json", ["sdlc"]],
    [".node-version", ["sdlc"]],
    ["tests/pr-check-scopes.test.js", ["sdlc"]],
  ...
  test("mixed changes take the union of their applicable checks", () => {
    const paths = ["scripts/set_up_sdlc.py", "mods/example/Mod.cs", "README.md"];
    paths.forEach((path) => write(path));
    commit(paths);
    expectSelection(["sdlc", "mods"]);
  });
  ...
  test("unusual filenames are passed through native Git without line parsing", () => {
    const path =
      process.platform === "win32"
        ? "mods/example/quoted ' and café.js"
        : "mods/example/quoted ' and café\nline.js";
    write(path);
    commit([path]);
    expectSelection(["mods"]);
  });
  ...
  test("a relevant input after more than 3000 other files is still selected", () => {
    ...
    paths.push("mods/example/last-relevant-input.js");
    paths.forEach((path) => write(path));
