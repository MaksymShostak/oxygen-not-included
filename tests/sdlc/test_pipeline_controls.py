...
class PullRequestValidationTests(unittest.TestCase):
    ...
    def test_r2_plan_baseline_uses_exact_prior_content_and_references(self):
        ...
        def validate(candidate=plan, prior=plan, paths=None):
            def blob(text):
                ...
                oid = hashlib.sha1(b'blob ' + str(len(raw)).encode('ascii') + b'\0' + raw).hexdigest()
                return BaselineBlob('example/service', 'a' * 40, fields['baseline'], '100644', oid, raw)
            return pull_request_validation.validate_pull_request_linkage(REPOSITORY, fields,
                'example/service', None, paths or ['tools/converter/src/index.js'], blob(candidate), blob(prior))
    ...
    def test_plan_fetch_uses_exact_revision_and_rejects_non_file_content(self):
        import base64
        text = '# Accepted plan\r\n'
        revision, root, docs, plans, blob = [letter * 40 for letter in 'abcde']
        raw = text.encode('utf-8')
        blob = hashlib.sha1(b'blob ' + str(len(raw)).encode('ascii') + b'\0' + raw).hexdigest()
        prefix = 'repos/example/service/git/'
        payloads = {
            prefix + 'commits/' + revision: {'sha': revision, 'tree': {'sha': root}},
            prefix + 'trees/' + root: {'sha': root, 'truncated': False, 'tree': [{'path': 'docs', 'mode': '040000', 'type': 'tree', 'sha': docs}]},
            prefix + 'trees/' + docs: {'sha': docs, 'truncated': False, 'tree': [{'path': 'plans', 'mode': '040000', 'type': 'tree', 'sha': plans}]},
            prefix + 'trees/' + plans: {'sha': plans, 'truncated': False, 'tree': [{'path': 'accepted-converter.md', 'mode': '100644', 'type': 'blob', 'sha': blob}]},
            prefix + 'blobs/' + blob: {'sha': blob, 'encoding': 'base64', 'size': len(text.encode()), 'content': base64.b64encode(text.encode()).decode()},
        }
        with patch.object(pull_request_validation, 'gh_api', side_effect=payloads.__getitem__) as api:
            fetched = pull_request_validation.fetch_baseline_at_revision(
                'example/service', 'docs/plans/accepted-converter.md', revision)
        self.assertEqual(fetched.raw, text.encode())
        self.assertEqual(api.call_args_list[0].args[0], prefix + 'commits/' + revision)
        ...
        payloads[prefix + 'trees/' + plans]['tree'][0]['mode'] = '120000'
        with patch.object(pull_request_validation, 'gh_api', side_effect=payloads.__getitem__):
            with self.assertRaises(SetupError):
                pull_request_validation.fetch_baseline_at_revision('example/service', 'docs/plans/accepted-converter.md', revision)
