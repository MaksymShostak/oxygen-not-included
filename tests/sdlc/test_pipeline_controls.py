...
class LocalVerificationControlTests(unittest.TestCase):
    ...
    def test_r2_uses_the_committed_plan_without_an_issue_snapshot(self):
        plan_path = 'docs/plans/accepted-converter.md'
        plan = self.repo / plan_path
        plan.parent.mkdir(parents=True)
        plan.write_text('# Converter\n\n## Preserve literal source\n', encoding='utf-8', newline='\n')
        self.commit('Prior accepted plan')
        self.begin_task('R2', plan_path)
        active = state.load_json(self.repo / state.ACTIVE_TASK_PATH)
        self.assertEqual(active['baseline'], plan_path)
        self.assertEqual(active['intentReference'], 'accepted-test-task')
        self.assertEqual(active['requiredProfiles'], ['full'])
        self.verify_task()
        self.assertEqual(state.required_verification_gaps(self.repo), [])
        plan.write_text('# Different intent\n', encoding='utf-8')
    ...
    def test_r2_refuses_new_or_changed_plan_baseline(self):
        plan_path = 'docs/plans/accepted-converter.md'
        plan = self.repo / plan_path
        plan.parent.mkdir(parents=True)
