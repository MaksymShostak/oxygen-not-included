...
def capture_issue_baseline(repo: Path, args: argparse.Namespace) -> None:
    ...
    destination = git_output_bytes(repo, 'remote', 'get-url', 'origin').decode().strip()
    issue = json.loads(run([github_cli, 'issue', 'view', str(args.issue), '--repo', destination, '--json', 'number,title,url,author,createdAt,updatedAt,labels,body'], cwd=repo, capture=True).stdout)
    repository = json.loads(run([github_cli, 'repo', 'view', destination, '--json', 'nameWithOwner'], cwd=repo, capture=True).stdout)['nameWithOwner']
...
def begin_task(repo: Path, args: argparse.Namespace) -> None:
    ...
    if args.baseline:
        ...
        if is_committed_plan_path(args.baseline):
...
def main() -> int:
    ...
    begin_parser.add_argument('--baseline', help='Accepted Issue snapshot JSON or committed docs/plans/*.md; R2/R3 require unchanged HEAD content.')
