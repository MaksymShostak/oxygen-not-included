...
def capture_issue_baseline(repo: Path, args: argparse.Namespace) -> None:
    ...
    destination = git_output_bytes(repo, 'remote', 'get-url', 'origin').decode('utf-8').strip()
    issue = json.loads(run([github_cli, 'issue', 'view', str(args.issue), '--repo', destination, '--json', 'number,title,url,author,createdAt,updatedAt,labels,body'], cwd=repo, capture=True, encoding='utf-8', errors='strict').stdout)
    repository = json.loads(run([github_cli, 'repo', 'view', destination, '--json', 'nameWithOwner'], cwd=repo, capture=True, encoding='utf-8', errors='strict').stdout)['nameWithOwner']
