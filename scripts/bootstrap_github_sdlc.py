...
from _repository import derive_repo_from_script, git_output
...
def main() -> int:
    ...
    try:
        ...
        destination = git_output(repo, 'remote', 'get-url', 'origin')
        if args.check_issue_readiness:
            result = run((gh, 'repo', 'view', destination, '--json', 'nameWithOwner,hasIssuesEnabled'),
        ...
        for name, color, description in LABELS:
            run((gh, 'label', 'create', name, '--repo', destination, '--color', color, '--description', description, '--force'), cwd=repo)
